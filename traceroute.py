...
def get_traceroute(qname):
	"""if len(sys.argv) != 2:
		print('usage: %s example.com' % sys.argv[0], file=sys.stderr)
		sys.exit(1)"""
	sk_args = (socket.AF_INET,
	           socket.SOCK_RAW,
	           socket.getprotobyname('icmp'))
	#host = socket.getaddrinfo(sys.argv[1],
	host = socket.getaddrinfo(qname,
	                          None,
	                          *sk_args)[0][4][0]
	#hop_limit = 32
	hop_limit = 64
	count = 3
	print('traceroute to %s (%s), %s hops max, 52 byte packets' %
	      (qname, host, hop_limit))
	
	recv_records=''
	recv_records+='traceroute to %s (%s), %s hops max, 52 byte packets\n' % (qname, host, hop_limit)
	...
	for ttl, seq_results in enumerate(results):
		...
		for result in seq_results:
			...
			if prev_addr != addr or addr == no_reply:
				recv_records+=str(' %s' % addr)
				print(' %s' % addr, end='')
			if addr != no_reply:
				recv_records+=str('  %0.3f ms' % (delay * 1e3))
...
if __name__ == '__main__':
	import sys
	sys.path.append("/Users/johndoe/python/functions/")
	import file
	qname="itunes.apple.com"
	recv_records=get_traceroute(qname)
	directory="/Users/johndoe/python/functions/traceroute/results/"
	#file.dir_make(directory)
	myfile=str(qname)+"_traceroute_"+time.strftime("%Y-%m-%d_%H-%M-%S", time.gmtime())+".txt"
	path=directory+myfile
	file.write_text(path,recv_records)
