...
def __get_show_result_parameters__(parameters):
	utility.print_violet2('Advanced test parameters')
	utility.print_yellow('Time: ' + time.ctime(parameters['Unix Epoch Time']) + ' timezone: ' + parameters.get('timezone', 'UTC'))
...
def __get_show_result_parameters_resolve__(parameters, host):
	print('\n\n\n')
	utility.print_grey('Resolve test host:')
	utility.print_yellow(host)
	print('Public IP Address: ', end='')
	#utility.print_violet(parameters)
	utility.print_violet(parameters['parameters']['Public IP Address'])
	print('Time:', time.ctime(parameters['parameters']['Unix Epoch Time']))
...
def __get_show_result_verbose_ping__(verbose_ping, host):
	...
	if statistics["packet_loss"]>0:
		print(utility.warp_grey('Ping test'),'host:', utility.warp_yellow(host), utility.warp_red('completed: False'))
		...
		for record in verbose_ping["ping"]:
			if record is None: utility.print_red('Request timeout for icmp_seq')
			else: print(f"{record[0]} bytes from {record[1]}: icmp_seq={record[2]} time={record[3]}")
		print('---', utility.warp_yellow(parameters['address']),'ping statistics ---')
		
		print(statistics['packets_transmitted'], 'packets transmitted', utility.warp_green(str(statistics['packets_received']) + ' packets received'), utility.warp_red(str(statistics['packet_loss']) + '% packet loss'))
		# print(statistics['packets_transmitted'], 'packets transmitted', utility.warp_green(str(statistics['packets_received']) + ' packets received'), utility.warp_red('statistics['packet_loss']'), '% packet loss'.format(statistics['packet_loss']))
		#print("{3} packets transmitted, {1}{4} packets received{2}, {0}{5:.1f}{2}".format(utility.Clr.RED,utility.Clr.GREEN,utility.Clr.RST,,,))
		print("\n")
	else: 
		print(utility.warp_grey('Ping test'), 'host:', utility.warp_yellow(host), utility.warp_green('completed: True'))
...
def __get_show_result_verbose_traceroute__(verbose_traceroute, host):
	...
	# host_reached = verbose_traceroute["host_reached"] if "host_reached" in verbose_traceroute else verbose_traceroute['statistics']["host_reached"]
	host_reached = verbose_traceroute.get('host_reached') or verbose_traceroute['statistics'].get('host_reached', None)
	# if verbose_traceroute["host_reached"]:
	if host_reached:
		print(utility.warp_grey('Traceroute test'), 'host:', utility.warp_yellow(host), utility.warp_green('host reached'))
	else:
		print(utility.warp_grey('Traceroute test'), 'host:', utility.warp_yellow(host), utility.warp_red('host reached'))
	if None in verbose_traceroute["traceroute"]:
		print(utility.warp_grey('Traceroute test'), 'host:', utility.warp_red2('luggage: False'))

		print(utility.warp_grey("https://www.freebsd.org/cgi/man.cgi?query=traceroute\n- \"TRACEROUTE System Manager's Manual:\n * * *\nA more interesting example is:\n% traceroute allspice.lcs.mit.edu.\n... only knows what's going on with * * *.\""))
		__get_show_traceroute__(verbose_traceroute)
	elif host_reached is False and not None in verbose_traceroute["traceroute"]: __get_show_traceroute__(verbose_traceroute)
...
def get_show_results(results):
	...
	#print(results['advanced_test']['dvprogram.state.gov']['resolve']['nslookup']['1.1.1.1']['A'])
	#print(results['advanced_test']['dvprogram.state.gov']['resolve']['parameters'])
	for host in results['advanced_test']:
		# print(results[host])
		# print(host)
		# print(results['advanced_test'][host]['resolve'])
		
		__get_show_result_parameters_resolve__(results['advanced_test'][host]['resolve'], host)
		#__get_show_result_resolve__(results['advanced_test'][host]['resolve'])
		__get_show_result_verbose_ping__(results['advanced_test'][host]['verbose_ping'], host)
		__get_show_result_verbose_traceroute__(results['advanced_test'][host]['verbose_traceroute'], host)


if __name__ == '__main__':
	# dir_parent =  os.path.dirname(__file__);
	# dir_results = "/results/";
	# file_result = "results_2022-05-18_16-35-48_hosts_all.json"
	...
	def get_show_results_listing():
		...
		for (enum,result) in enumerate(listing_results, start=0):
			...
			print("Number #:"+str(enum),result.stem)
		enum_input = ""
		while True:
			# enum_input = input("Type e -for EXIT, Type Number (show result) #: 0 - " + str(len(listing_results)-1) + str("\n") + str("Number #:"))
			enum_input = input('Type e -for EXIT, Type Number (show result) #: 0 - ' + str(len(listing_results)) + str('\n') + str('Number #:'))
			if enum_input is 'e': break
			elif int(enum_input) >= len(listing_results): break
			else: 
				get_contents=(listing_results[int(enum_input)])
				#print(File.open_json(get_contents))
				return (File.open_json(get_contents))
				#break
			#elif int(enum_input)<len(listing_results): break
